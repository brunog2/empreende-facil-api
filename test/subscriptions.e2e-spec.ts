...
describe("Subscription endpoints (e2e)", () => {
  ...
  const plansService = {
    findActive: jest
      .fn()
      .mockResolvedValue([{ code: "trial", isActive: true }]),
  ...
  const subscriptionsService = {
    getMine: jest
      .fn()
      .mockResolvedValue({ id: "subscription-id", status: "active" }),
    getUsage: jest
      .fn()
      .mockResolvedValue({ products: { current: 0, limit: 30 } }),
    getPayments: jest.fn().mockResolvedValue([]),
    checkout: jest
      .fn()
      .mockResolvedValue({ code: "PAYMENT_CONFIRMATION_PENDING" }),
    changePlan: jest.fn(),
    cancel: jest.fn(),
    reactivate: jest.fn(),
  };
  ...
  it("GET /api/plans retorna apenas a coleção do service público", async () => {
    await request(app.getHttpServer())
      .get("/api/plans")
      .expect(200)
      .expect([{ code: "trial", isActive: true }]);
  });
  ...
  it("GET /api/subscriptions/me mantém o escopo do usuário autenticado", async () => {
    await request(app.getHttpServer())
      ...
      .expect(200)
      .expect({ id: "subscription-id", status: "active" });
